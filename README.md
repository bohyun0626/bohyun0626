...
#### 📌 프로젝트명: MES 연동을 위한 SP 로직 개발 및 데이터 자동 동기화 구현
#### 기간: 2025.01(1개월)
#### 사용 기술/도구: SSMS (SQL Server Management Studio)
#### 주요 업무 및 역할: <br>- ERP DB 내 품목 정보 및 작업지시 내역의 저장, 조회, 삭제 내역을 주기적으로 추출하는 SP 로직을 신규 개발<br>- SP를 통해 수집된 데이터가 MES와 정기적으로 동기화되도록 연동 로직 구현<br>- 스케줄링 기반 연동 시나리오 설계: 일정 시간마다 SP 호출 → 최신 데이터 전달<br>- SP 성능 테스트 및 오류 로그 분석을 통한 안정성 확보<br>- 개발 완료 후 MES 시스템 담당자와 데이터 연동 정상 작동 여부를 검증
#### 결과 및 성과: <br>- ERP ↔ MES 간 데이터 일치율 100% 확보<br>- MES 측의 수작업 등록 프로세스 완전 제거, 실시간 데이터 조회 환경 구축<br>- 향후 공정 자동화 확장의 기반 마련
...
2024.09-2025.01 전기회사의 ERP 컨버전PJT: 기존 CS버전의 ERP시스템에서 웹 버전의 시스템으로 컨버전. 버전 차이로 인한 오류, 프로시저의 오류 등을 수정하고, 신규화면을 약 20건 개발하였으며 고객사 유구에 따른 업그레이드 진행. 추가적으로 오즈리포트로 출력물 개발

2024.01 식품회사의 데이터 연동SP 개발: 품목정보, 작업지시정보를 MES와 연동하는 SP로직 구현
